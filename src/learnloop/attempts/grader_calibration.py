...
from learnloop.diagnosis.probe_families import GRADER_CHANNEL_RELIABILITY
            ...
            gi_model_id = repository.insert_calibration_model(
                model={
                    ...
                    **gi_scope,
                    ...
                    "backoff_chain_json": canonical_json([model_id]),
                    "status": "heuristic",
                    "count_heuristic_prior": int(PRIOR_CONCENTRATION),
                    "prior_concentration": PRIOR_CONCENTRATION,
                    "provenance_json": canonical_json(
...
def _uniform_fallback(
    ...
    return ResolvedModel(
        model_id="",
        model_hash=canonical_hash({"fallback": True, "schema": outcome_schema_id}),
        joint_alpha=alphas,
...
def credible_interval(
    ...
    *,
    ...
    if prior is None:
        prior = {z: 1.0 / len(classes) for z in classes}
    seed = int(canonical_hash({"h": resolved.model_hash, "e": emission})[:12], 16)
