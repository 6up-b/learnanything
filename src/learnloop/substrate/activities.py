...
from learnloop.learner.assessment_contracts import compile_assessment_contract
...
def card_contract_hash(contract: Mapping[str, Any], *, purpose: str) -> str:
    return canonical_hash(card_semantic_payload(contract, purpose=purpose))


def surface_hash(contract: Mapping[str, Any]) -> str:
    return canonical_hash(surface_payload(contract))
...
def resolve_legacy_item(
    ...
    *,
    ...
    family_version_id = repository.ensure_activity_family_version(
        family_id=family_id,
        version=1,
        family_spec_json=canonical_json(
            {
                "purpose": purpose,
    ...
    card_version_id = repository.ensure_activity_card_version(
        ...
        contract_json=canonical_json(card_semantic_payload(contract, purpose=purpose)),
    ...
    surface_id = repository.ensure_activity_surface(
        ...
        surface_hash=sh,
        fingerprint=fp,
        surface_json=canonical_json(surface_payload(contract)),
    ...
    *,
    ...
    try:
        reservation_id = repository.insert_surface_reservation(
            ...
            purpose=purpose,
            eligibility_json=canonical_json(eligibility.as_dict()),
            clock=clock,
        )
    ...
    repository.append_surface_lifecycle_event(
        ...
        detail_json=canonical_json({"purpose": purpose, "eligibility": eligibility.as_dict()}),
        clock=clock,
    )
    return Reservation(
        reservation_id=reservation_id,
...
def open_administration(
    ...
    *,
    ...
    pins = {
        ...
        "selection_policy_version_id": selection_policy_version_id,
        "decision_params_hash": decision_params_hash,
        "assistance_json": canonical_json(dict(assistance)) if assistance is not None else None,
        "feedback_condition": feedback_condition,
        "eligibility_json": canonical_json(eligibility_dict),
    }
...
def append_practice_successor_proposal(
    ...
    *,
    ...
    return repository.append_surface_lifecycle_event(
        ...
        reason=reason,
        detail_json=canonical_json({"stage": "proposal", "not_before": not_before}),
        clock=clock,
    )
...
def append_exposure(
    ...
    *,
    ...
    return repository.append_exposure_event(
        ...
        purpose=purpose,
        consumes_unseen=consumes_unseen,
        detail_json=canonical_json(dict(detail)) if detail is not None else None,
        clock=clock,
    )
...
def append_feedback(
    ...
    *,
    ...
    return repository.append_exposure_event(
        ...
        detail_json=canonical_json({"timing": timing}),
        clock=clock,
    )
...
def retire_with_reason(
    ...
    *,
    ...
    interaction_event_id = repository.append_interaction_event(
        ...
        surface_id=surface_id,
        payload_json=canonical_json(detail),
        clock=clock,
    )
    return repository.insert_retirement_record(
        ...
        reason=reason,
        provenance=provenance,
        replacement_proposal_json=canonical_json(dict(replacement_proposal))
...
def log_interaction_event(
    ...
    *,
    ...
    from learnloop.attempts.salience_firewall import READING_EVENT_KINDS, SALIENCE_ONLY
    ...
    return repository.append_interaction_event(
        ...
        payload_json=canonical_json(dict(payload)) if payload is not None else None,
        clock=clock,
    )
...
def author_family_version(
    ...
    *,
    ...
    family_version_id = repository.ensure_activity_family_version(
        family_id=family_id, version=version, family_spec_json=canonical_json(dict(family_spec)), clock=clock
    )
    ...
    repository.upsert_activity_family_authoring(
        ...
        fields={
            ...
            "served_milestone_edges_json": canonical_json(served_milestone_edges) if served_milestone_edges is not None else None,
            "cross_purpose_links_json": canonical_json(links) if links else None,
            "angle_inventory_json": canonical_json(angle_inventory) if angle_inventory is not None else None,
            "coverage_targets_json": canonical_json(coverage_targets) if coverage_targets is not None else None,
            "status": "draft",
        },
        clock=clock,
    )
...
def link_cross_purpose_families(
    ...
    *,
    ...
    fields["cross_purpose_links_json"] = canonical_json(merged)
    repository.upsert_activity_family_authoring(
    ...
    *,
    ...
    repository.upsert_activity_card_authoring(
        ...
        fields={
            ...
            "task_features_json": canonical_json(dict(task_features)) if task_features is not None else None,
            "capability": capability,
            "outcome_schema_id": outcome_schema_id,
            "outcome_schema_version": outcome_schema_version,
            "surface_policy": surface_policy,
            "surface_variation_bounds_json": canonical_json(dict(surface_variation_bounds)) if surface_variation_bounds is not None else None,
            "angle_identity_json": canonical_json(dict(angle_identity)) if angle_identity is not None else None,
            "generator_version": generator_version,
            "gate_policy_version": gate_policy_version,
            "expected_burden_json": canonical_json(dict(expected_burden)) if expected_burden is not None else None,
            "calibration_metadata_json": canonical_json(dict(calibration_metadata)) if calibration_metadata is not None else None,
        },
        clock=clock,
    )
