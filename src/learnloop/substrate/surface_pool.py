    ...
    *,
    ...
    for surface in _canonical_surfaces(surfaces):
        repository.register_practice_pool_surface(
            ...
            content_hash=canonical_hash(surface),
            clock=clock,
        )
...
def review_pool(
    ...
    *,
    ...
    repository.transition_practice_pool(
        ...
        detail_json=canonical_json(dict(checks)) if checks else None, author=author, clock=clock,
    )
    return _load_pool(repository, pool_id)
...
def _record_pool_selection(
    ...
    if selection.rotated:
        repository.append_practice_pool_event(
            ...
            detail_json=canonical_json({"reason": selection.reason, "fallback": selection.fallback}),
            clock=clock,
        )
    repository.append_practice_pool_event(
        pool_id=pool_id,
