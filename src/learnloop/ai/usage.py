...
This provider-independent accounting lives with the shared AI transport
contracts.
"""
...
class TokenUsageAccounting:
    ...
    them raise ``AIProviderUnavailable`` partway through, so a client that never
...
def consume_client_usage(client: Any | None) -> TokenUsage:
    ...
    Duck-typed on purpose. Usage is optional on the operation-client contract,
    and grading/authoring services accept compact in-process test transports as
    well as provider clients and sidecar adapters. A stub without the method
