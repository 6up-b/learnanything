...
from learnloop.ai.routing import runtime_for_provider
...
from learnloop.diagnosis.error_taxonomy_map import map_legacy_error_type
from learnloop.substrate.state_sync import StateSyncResult, sync_vault_state
...
class DoctorReport:
    ...
    codex_runtime: AIRuntimeReport | None = None
    ai_runtime: AIRuntimeReport | None = None
    deprecated_table_row_counts: dict[str, int] = field(default_factory=dict)
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "deprecated_table_row_counts": self.deprecated_table_row_counts,
...
def run_doctor(root: Path, *, fix_state: bool = False, ai: bool = False, ai_provider: str | None = None) -> DoctorReport:
    ...
    if ai:
        selected_provider = (
            ai_provider
            or os.environ.get("LEARNLOOP_AI_PROVIDER")
            or config.ai.active_provider
        )
        ai_runtime = runtime_for_provider(vault_root, config, selected_provider)
        if selected_provider in CODEX_PROVIDER_NAMES:
            # Compatibility field for the existing JSON contract. It mirrors
            # the one explicitly requested diagnostic; plain doctor never
            # probes a provider or starts a provider process.
            codex_runtime = ai_runtime
    ...
    if fix_state:
        migrate_vault(paths.root, paths.sqlite_path)
    database_readable = _check_sqlite(paths, issues)

    repository: Repository | None = None
    database_tables: set[str] = set()
    deprecated_table_row_counts: dict[str, int] = {}
    if database_readable:
        try:
            repository = Repository.attach(
                paths.sqlite_path,
                read_only=not fix_state,
            )
            database_tables = _database_tables(repository)
        except sqlite3.Error as exc:
            issues.append(
                _issue(
                    "error",
                    "sqlite:unreadable",
                    f"SQLite state database could not be read: {exc}",
                    paths.sqlite_path,
                )
            )
            repository = None
    if repository is not None:
        deprecated_table_row_counts = _check_deprecated_tables(
            repository,
            database_tables,
            issues,
        )
    _recover_apply_intents(
        paths,
        repository,
        database_tables,
        issues,
    ...
    try:
        ...
        return DoctorReport(
            ...
            deprecated_table_row_counts=deprecated_table_row_counts,
        )

    issues.extend(_from_loader_issue(issue) for issue in vault.issues)
    _check_references(vault, issues)

    _check_source_sets(vault, repository, database_tables, issues)
    state_sync_result = (
        sync_vault_state(vault, repository)
        if fix_state and repository is not None
        else None
    )
    if (
        fix_state
        and repository is not None
        and "evidence_facet_recall_state" in database_tables
    ...
    if repository is not None:
        ...
        if _has_tables(database_tables, "practice_attempts", "derived_state_rebuilds"):
            _check_derived_state_rebuild_marker(vault, repository, issues)
        if "proposed_patch_items" in database_tables:
            _check_invalid_proposals(repository, issues)
        if _has_tables(database_tables, "practice_attempts", "attempt_surprise"):
            _check_difficulty_calibration(vault, repository, issues)
        if "practice_item_quality_state" in database_tables:
    ...
    if repository is not None:
        if _has_tables(database_tables, "proposed_patches", "proposed_patch_items"):
            _check_duplicate_diagnostic_proposals(vault, repository, issues)
        if _has_tables(database_tables, "goal_contract_heads", "goal_contract_versions"):
            _check_contract_drift(vault, repository, issues)
        if _has_tables(database_tables, "facet_merges", "facet_recall_state"):
        ...
        if _has_tables(
            ...
            _check_pre_first_practice_identifiability(
                vault,
                repository,
                issues,
                fix_state=fix_state,
...
def _check_retired_config(root: Path, issues: list[HealthIssue]) -> None:
    ...
    if "codex" in data:
        issues.append(
            _issue(
                "warning",
                "config:legacy_codex_translated",
                "The deprecated [codex] table was translated in memory to "
                "[ai.providers.codex]; rewrite it using the canonical provider table.",
                path,
                details={
                    "from": "codex",
                    "to": "ai.providers.codex",
    ...
    if (
        ...
        issues.append(
            _issue(
                "warning",
                "config:legacy_audio_transcription_translated",
                "The legacy [ingest.audio] provider = \"openrouter\" setting was "
                "translated in memory to a dedicated AI profile and the "
                "transcription task route; rewrite it using canonical [ai] tables.",
                path,
                details={
                    "from": "ingest.audio.provider",
                    "to": f"ai.providers.{OPENROUTER_TRANSCRIPTION_PROVIDER}",
    ...
    for setting_path in retired_setting_paths:
        value: Any = data
        for segment in setting_path:
            ...
            issues.append(
                _issue(
                    ...
                    f"learnloop.toml sets retired '{dotted}'; the value is ignored and should be removed.",
                    path,
                    details={"setting": dotted, "action": "remove"},
                )
            )

    legacy_codex = data.get("codex")
    auth_locations: list[str] = []
    if isinstance(legacy_codex, dict) and "auth_mode" in legacy_codex:
        auth_locations.append("codex.auth_mode")
    ai = data.get("ai")
    providers = ai.get("providers") if isinstance(ai, dict) else None
    if isinstance(providers, dict):
        auth_locations.extend(
            f"ai.providers.{name}.auth_mode"
            for name, profile in providers.items()
            if isinstance(profile, dict) and "auth_mode" in profile
        )
    for location in auth_locations:
        issues.append(
            _issue(
                "warning",
                "config:retired_auth_mode",
                f"learnloop.toml sets retired '{location}'; authentication is provider-native and this value is ignored.",
                path,
                details={"setting": location, "action": "remove"},
            )
        )
...
def _check_sqlite(paths: VaultPaths, issues: list[HealthIssue]) -> bool:
...
def _database_tables(repository: Repository) -> set[str]:
...
def _check_deprecated_tables(
    repository: Repository,
    database_tables: set[str],
    issues: list[HealthIssue],
) -> dict[str, int]:
    """Report read-only telemetry before deprecated state is detached.

    The abandonment hypothesis is only safe while these tables are empty.  A
    non-zero count is therefore a stop-and-escalate warning, not permission to
    delete or migrate the rows.
    """

    counts: dict[str, int] = {}
    try:
        with repository.connection() as connection:
            for table in _DEPRECATED_STATE_TABLES:
                ...
                if count:
                    issues.append(
                        _issue(
                            "warning",
                            "sqlite:deprecated_table_not_empty",
                            f"Deprecated table {table} contains {count} row(s); "
                            "stop deprecation work and escalate before detaching any code or schema.",
        ...
        issues.append(
            _issue(
                "error",
                "sqlite:deprecated_table_check_failed",
                f"Deprecated-table row counts could not be read: {exc}",
                repository.sqlite_path,
            )
        )
    return counts
...
def _has_tables(available: set[str], *required: str) -> bool:
    return set(required).issubset(available)


def _recover_apply_intents(
    paths: VaultPaths,
    ...
    *,
    ...
    if repository is None or "apply_intents" not in database_tables:
        return
...
def _check_source_sets(
    ...
    from learnloop.content.sources.role_authority import KNOWN_ROLES

    subject_ids = set(vault.subjects)
    db_available = repository is not None and _has_tables(
        database_tables,
        "source_artifacts",
        "source_revisions",
    )
...
def _check_blueprints_and_criteria(vault: LoadedVault, issues: list[HealthIssue]) -> None:
    ...
    from learnloop.learner.capability_mapping import is_valid_capability
...
def _check_instrument_contracts(vault: LoadedVault, issues: list[HealthIssue]) -> None:
    ...
    from learnloop.learner.capability_mapping import is_valid_capability
    from learnloop.content.authoring.persona_gate import declares_error_count
...
def _check_pre_first_practice_identifiability(
    ...
    from learnloop.learner.identifiability import calculate_registry_hash
    ...
    for subject_id in sorted(vault.subjects):
        ...
        registry_hash = calculate_registry_hash(view)
...
def _check_contract_drift(vault, repository, issues: list[HealthIssue]) -> None:
    ...
    from learnloop.goals.goal_contracts import detect_contract_drift
