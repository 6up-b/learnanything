...
- ``apply_config_updates`` — comment-preserving structured edits to the
  ...
  ``learnloop.ops.vault_upgrade._rewrite_algorithm_version``.
...
from learnloop.config import ENV_KEY_RE, AIProviderConfig, CODEX_PROVIDER_NAMES
...
def copy_ai_settings(source_path: Path, target_path: Path) -> bool:
    ...
    known_tasks = {
        task for routes in USE_CASE_ROUTES.values() for task in routes
    } | {"transcription"}
...
def upsert_env_var(path: Path, key: str, value: str | None) -> None:
    ...
    prefix, ``#`` comments, ``ENV_KEY_RE`` names). All other lines are kept
