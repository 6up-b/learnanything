...
def upgrade_to_mvp08(root: Path, *, clock: Clock | None = None) -> UpgradeResult:
    ...
    from learnloop.params import parameter_registry as registry_service
    from learnloop.substrate.p0_projection import activate_p0_projection
...
def upgrade_to_mvp09(root: Path, *, clock: Clock | None = None) -> UpgradeResult:
    """Activate cross-channel reveal accounting on the existing P0 projection.

    ``mvp-0.9`` changes how future answer exposure is recorded, not the
    authority-propagation projection introduced in ``mvp-0.8``. The upgrade is
    therefore an immediate-successor tag change plus an idempotent rebuild of
    that same projection namespace. Raw attempts and reveal history are never
    rewritten.
    """
    ...
    vault = load_vault(root)
    current = vault.config.algorithms.algorithm_version
    if current == REVEAL_LEDGER_ALGORITHM_VERSION:
        return UpgradeResult(
            upgraded=False,
            from_version=current,
            to_version=REVEAL_LEDGER_ALGORITHM_VERSION,
            problems=["vault is already mvp-0.9"],
        )
    if current != P0_ALGORITHM_VERSION:
        return UpgradeResult(
            upgraded=False,
            ...
            to_version=REVEAL_LEDGER_ALGORITHM_VERSION,
            problems=[
                f"cannot upgrade from {current!r}; only {P0_ALGORITHM_VERSION!r} may activate mvp-0.9"
            ],
        )

    sqlite_path = VaultPaths(vault.root, vault.config).sqlite_path
    repository = Repository(sqlite_path)
    registry_service.freeze_manifest(
        vault,
        repository,
        algorithm_version=P0_ALGORITHM_VERSION,
        clock=clock,
    )
    vault.config.algorithms.algorithm_version = REVEAL_LEDGER_ALGORITHM_VERSION
