...
def advance(
    ...
    *,
    ...
    result = repository.append_golden_path_run_event(
        ...
        feasible_alternatives_json=canonical_json(list(feasible_alternatives)) if feasible_alternatives else canonical_json(sorted(allowed)),
        evidence_ids_json=canonical_json(list(evidence_ids)) if evidence_ids else None,
        ...
        selected_activity_json=canonical_json(dict(selected_activity)) if selected_activity else None,
        policy_calibration_json=canonical_json(dict(policy_calibration)) if policy_calibration else None,
        burden_json=canonical_json(dict(burden)) if burden else None,
        clock=clock,
    )
