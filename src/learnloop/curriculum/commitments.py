...
def target_set_hash(targets: Sequence[CommitmentTarget]) -> str:
    ...
    return canonical_hash(sorted((t.normalized() for t in targets), key=canonical_json))
    ...
    *,
    ...
    payload = {
        **{k: v for k, v in fields.items() if k != "version_hash"},
        "targets": sorted((t.normalized() for t in targets), key=canonical_json),
...
def create_commitment(
    ...
    *,
    ...
    policy_id = repository.ensure_depth_policy_version(
        ...
        body_json=canonical_json(policy_body),
        content_hash=canonical_hash(policy_body),
        clock=clock,
    )
    ...
    envelope_id = repository.ensure_depth_envelope_version(
        ...
        bounds_json=canonical_json(envelope_body["bounds"]),
        reviewed_edges_json=canonical_json(envelope_body["reviewed_edges"]),
        content_hash=canonical_hash(envelope_body),
        clock=clock,
    )
    ...
    version_fields = {
        ...
        "attention_bounds_json": canonical_json(dict(attention_bounds)) if attention_bounds else None,
        "due_hint": due_hint,
        "hiatus_hint": hiatus_hint,
        "reason": reason,
        "provenance_json": canonical_json(dict(provenance)) if provenance else None,
        "target_set_hash": ts_hash,
    }
...
def _target_row(t: CommitmentTarget) -> dict[str, Any]:
    return {
        ...
        "role": t.role,
        "salience": t.salience,
        "provenance_json": canonical_json(t.provenance) if t.provenance else None,
    }
...
def change_disposition(
    ...
    *,
    ...
    repository.append_commitment_event(
        ...
        detail_json=canonical_json({"disposition": disposition}),
        clock=clock,
    )
...
def satisfy_single_check(
    ...
    repository.append_commitment_event(
        ...
        commitment_version_id=None,
        kind="disposition_changed",
        detail_json=canonical_json({"disposition": "satisfied", "administration_id": administration_id}),
        clock=clock,
    )
    return resolve_disposition(repository, commitment_id)
...
def record_depth_transition_committed(
    ...
    *,
    ...
    return repository.append_commitment_event(
        ...
        detail_json=canonical_json(dict(detail)) if detail else None,
        clock=clock,
    )
...
def attach_family(
    ...
    repository.append_commitment_event(
        ...
        detail_json=canonical_json({"family_id": family_id}), clock=clock,
    )
