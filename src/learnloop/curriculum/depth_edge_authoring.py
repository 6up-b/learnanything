...
from learnloop.substrate.activity_patterns import (
    LEGACY_UNMAPPED,
...
from learnloop.curriculum.depth_rungs import project_task_contract
...
def request_depth_edge_instances(
    client: StructuredTransport, context: DepthEdgeInstanceContext
) -> DepthEdgeInstanceBatch:
    """Author depth-edge candidates through the shared transport."""

    return execute_structured_operation(
        client,
        purpose="depth_edge_instances",
        prompt=depth_edge_instance_prompt(context),
        result_model=DepthEdgeInstanceBatch,
    )
...
def create_edge_template(
    ...
    *,
    ...
    version_id = repository.insert_depth_edge_template_version(
        ...
        content_hash=canonical_hash(dict(body)),
        clock=clock,
    )
...
def author_edge_instances(
    ...
    *,
    ...
    batch = request_depth_edge_instances(client, context)
