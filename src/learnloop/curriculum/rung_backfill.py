...
def request_rung_backfill(
    client: StructuredTransport, context: RungBackfillContext
) -> RungBackfillClassification:
    """Classify legacy item rungs through the shared transport."""

    return execute_structured_operation(
        client,
        purpose="rung_backfill",
        prompt=rung_backfill_prompt(context),
        result_model=RungBackfillClassification,
    )
...
def backfill_item_rungs(
    ...
    *,
    ...
    for start in range(0, len(targets), batch_size):
        ...
        result = request_rung_backfill(client, context)
