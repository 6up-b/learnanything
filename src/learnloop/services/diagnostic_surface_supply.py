...
def probe_pool_empty_conditions(
    ...
    * a pending diagnostic replenishment need's LO has zero fresh active
    ...
    from learnloop.services.probe_episodes import administered_surface_exclusions
...
def authoring_provider_available(vault: LoadedVault) -> str | None:
    ...
    selection = provider_for_task(vault.config, "authoring")
    name = (selection.provider_name or "").strip()
    if not name or name == "manual":
        return None
    try:
        if name == "codex" and name not in vault.config.ai.providers:
            ...
            report = check_ai_runtime(vault.root, vault.config, provider_name=name)
    except Exception:  # pragma: no cover - a broken runtime probe means "not ready"
        return None
    return name if report.status == "ready" else None
