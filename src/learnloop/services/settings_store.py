...
Provider choices are stored in each vault's ``learnloop.toml`` while API keys
live in the machine-global ``settings.env``. TOML updates preserve comments and
layout; both write paths replace a temporary sibling atomically.
"""
...
# Coarse UI use cases expand to the concrete task routes consumed by
# ``provider_for_task``.
...
def openrouter_task_profile_values(base: AIProviderConfig, model: str) -> dict[str, Any]:
    """Return the explicitly configured fields for one OpenRouter task profile."""
...
def apply_config_updates(config_path: Path, updates: Mapping[tuple[str, ...], Any]) -> None:
    """Set TOML key paths while preserving comments and unrelated formatting."""
...
def copy_ai_settings(source_path: Path, target_path: Path) -> bool:
    """Copy explicitly persisted AI routes and non-Codex profiles to a new vault."""
    ...
    try:
        ...
        raise SettingsStoreError(
            "config_unreadable",
            f"{source_path} is not valid TOML: {exc}",
        ) from exc
    ...
    if isinstance(providers, Mapping):
        for name, table in providers.items():
            if name in CODEX_PROVIDER_NAMES or not isinstance(table, Mapping):
                continue
            _flatten_into_updates(("ai", "providers", str(name)), table, updates)
...
def _flatten_into_updates(
    ...
    for key, value in table.items():
        if isinstance(value, Mapping):
            _flatten_into_updates((*prefix, str(key)), value, updates)
        else:
            updates[(*prefix, str(key))] = value


def upsert_env_var(path: Path, key: str, value: str | None) -> None:
    """Set or remove one key in a dotenv-style file without touching other lines."""
    ...
    for line in lines:
        ...
        name, separator, _rest = candidate.partition("=")
        is_target = (
            not replaced
            and separator == "="
