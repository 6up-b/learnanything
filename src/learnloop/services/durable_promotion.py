...
``misconceptions._promote_candidate``.
...
#: first real producer of an A6 withdrawal (`services/surfaced_beliefs.py`).
#:
...
#:   `_promote_candidate` freezes an authored correction at promotion time.
...
_ANY_BELIEF_STATUS = ("active", "resolving", "resolved", "demoted", "superseded")
...
def _promote_hypothesis(
    ...
    *,
    ...
    from learnloop.services.causal_attribution import _trace_consistent
    from learnloop.services.misconceptions import _normalize_text, _promote_candidate
    ...
    normalized = str(hypothesis.get("statement_normalized") or "") or _normalize_text(
    ...
    for record in repository.misconceptions_for_learning_object(
        ...
        if _normalize_text(record.statement) != normalized:
            continue
...
def _withdraw_hypothesis(
    ...
    *,
    ...
    from learnloop.services.surfaced_beliefs import record_belief_withdrawal
