...
Every constraint reads ONLY from the :class:`~learnloop.scheduling.controller_snapshot`
...
here -- they are P4 step 4 (``learnloop.scheduling.dispersion`` /
...
if TYPE_CHECKING:  # avoid an import cycle: the snapshot imports nothing from here
    from learnloop.scheduling.controller_snapshot import Candidate, ControllerSnapshot
    from learnloop.scheduling.staged_policy import AttentionBlock
...
def manifest() -> dict[str, Any]:
    ...
    return {"definitions": definitions, "manifest_hash": canonical_hash(body),
