...
from learnloop.substrate.activities import canonical_json
    ...
    *,
    ...
    with repository.connection() as connection:
        ...
        connection.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                snapshot_id, snapshot_hash, session_id, canonical_json(dict(body)),
                param_manifest_hash, canonical_json(dict(projection_versions)), utc_now_iso(clock),
            ),
        )
        connection.commit()
    return snapshot_id
    ...
    *,
    ...
    with repository.connection() as connection:
        ...
        connection.execute(
            ...
            (manifest_id, manifest_hash, canonical_json(definitions), utc_now_iso(clock)),
        )
        connection.commit()
    return manifest_id
    ...
    with repository.connection() as connection:
        connection.execute(
            ...
            (
                ...
                canonical_json(dict(neighborhood)), canonical_json(exit_rules), short_circuit_reason,
    ...
    try:
        ...
        connection.execute(
            ...
            (event_id, block_id, ordinal, kind,
             canonical_json(dict(detail)) if detail is not None else None, utc_now_iso(clock)),
        )
        connection.commit()
    except Exception:
        connection.rollback()
    ...
    try:
        ...
        connection.execute(
            ...
            (
                ...
                canonical_json(dict(comparator)) if comparator is not None else None,
                canonical_json(dict(trace)), now,
            ),
        )
        for cand in candidates:
            connection.execute(
                ...
                (
                    ...
                    canonical_json(cand.get("exclusion_reasons", [])),
                    canonical_json(cand.get("within_mode_metrics", {})),
    ...
    with repository.connection() as connection:
        connection.execute(
            ...
            (
                ...
                canonical_json(dict(prediction)), 1 if usable else 0, utc_now_iso(clock),
            ),
        )
        connection.commit()
    ...
    with repository.connection() as connection:
        connection.execute(
            ...
            (
                ...
                canonical_json(candidate_refs if candidate_refs is not None else []),
                canonical_json(dict(detail) if detail is not None else {}), utc_now_iso(clock),
            ),
        )
        connection.commit()
    ...
    *,
    ...
    with repository.connection() as connection:
        connection.execute(
            ...
            (status, canonical_json(dict(outcome)), utc_now_iso(clock), window_id),
        )
        connection.commit()
