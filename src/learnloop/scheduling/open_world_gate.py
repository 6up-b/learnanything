...
def _c3_p2(repository: Repository) -> Condition:
    ok, detail = _has_attrs("learnloop.curriculum.golden_path_assessment", ("__name__",))
    ok2, detail2 = _has_attrs("learnloop.curriculum.pattern_ladder", ("__name__",))
    met = ok and ok2
    return Condition(
...
def _c4_p3(repository: Repository) -> Condition:
    ok, detail = _has_attrs("learnloop.reader.annotations", ("__name__",))
    ok2, detail2 = _has_attrs("learnloop.attempts.salience_firewall", ("__name__",))
    met = ok and ok2
    return Condition(
...
def _c5_controller(repository: Repository) -> Condition:
    ok_s, detail_s = _has_attrs("learnloop.scheduling.constraint_engine", ("evaluate", "manifest"))
    ok_p, detail_p = _has_attrs("learnloop.scheduling.staged_policy", ("decide",))
...
def _c6_dispersion_kernel(repository: Repository) -> Condition:
    ok_d, _ = _has_attrs("learnloop.scheduling.dispersion", ("__name__",))
    ok_i, _ = _has_attrs("learnloop.scheduling.interleaving", ("__name__",))
    ...
    from learnloop.scheduling import kinship_feature
