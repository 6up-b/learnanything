...
def _pairs_for(
    ...
    for pred in _component_predictions(repository, scorer_kind):
        ...
        outcome = canonical_json.loads(window["outcome_json"])
        if outcome_key not in outcome:
            continue
        predicted = canonical_json.loads(pred["prediction_json"]).get("value")
    ...
    with repository.connection() as connection:
        ...
        connection.execute(
            ...
            (
                ...
                canonical_json.dumps(report.metrics), canonical_json.dumps(report.splits),
