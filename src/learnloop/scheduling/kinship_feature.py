...
def _features_for(repository: Repository, surface_id: str) -> dict[str, Any]:
    ...
    return canonical_json.loads(row["features_json"]) if row is not None else {}
    ...
    with repository.connection() as connection:
        ...
        connection.execute(
            ...
            (
                ...
                canonical_json.dumps(dict(scope or {"scope": "learner_local"})),
                canonical_json.dumps(dict(consent or {"consent": "not_required_shadow"})),
                now,
            ),
        )
        connection.execute(
            ...
            (new_ulid(), model_id, canonical_json.dumps({"note": "model created shadow"}), now),
        )
        connection.commit()
        return model_id
    ...
    with repository.connection() as connection:
        connection.execute(
            ...
            (
                ...
                canonical_json.dumps(score.as_dict()), canonical_json.dumps(score.conditioned_on),
...
def consulted_discount(
    ...
    *,
    ...
    if row is None:
        return p1_conservative_discount
    outputs = canonical_json.loads(row["outputs_json"])
    ...
    *, clock: Clock | None,
    ...
    with repository.connection() as connection:
        ...
        connection.execute(
            ...
            (new_ulid(), model_id, ordinal, kind, canonical_json.dumps(dict(detail)),
             utc_now_iso(clock)),
        )
        connection.commit()
