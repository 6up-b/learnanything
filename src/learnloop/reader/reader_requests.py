...
def request_reader_preset_synthesis(
    client: StructuredTransport, context: ReaderPresetSynthesisContext
) -> ReaderPresetSynthesis:
    """Synthesize one reader preset through the shared transport."""

    return execute_structured_operation(
        client,
        purpose="reader_preset_synthesis",
        prompt=reader_preset_synthesis_prompt(context),
        result_model=ReaderPresetSynthesis,
    )
...
def model_synthesis(client: Any) -> Callable[[Repository, Mapping[str, Any], Clock | None], dict[str, Any]]:
    ...
    (§6.4). A provider lacking structured completion raises so the
    ...
    def synthesize(
        ...
        result = request_reader_preset_synthesis(
            ...
            ReaderPresetSynthesisContext(
                ...
                section_paths=[
                    list(path)
                    for path in window.get("section_paths") or []
