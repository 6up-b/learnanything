...
from learnloop.content.pipeline.source_ingestion import (
    SourceChunk,
    caption_chunks_for_time_range,
    child_chunks_for_locator,
    chunks_for_note_body,
    locator_hash_for_ref,
    source_youtube_video_id,
)
from learnloop.content.sources.source_refs import source_ref_presentation
...
def resolve_source_refs(vault, item, repository=None) -> list[dict[str, Any]]:
    ...
    for ref in item.provenance.source_refs:
        ...
        if note is None:
            ...
            continue
        chunks = chunks_for_note_body(kind, note.body)
        if kind == "youtube_video":
...
def _resolve_text_ref(ref, note, canonical: dict[str, Any], kind: str, chunks: list[SourceChunk]) -> dict[str, Any]:
    ...
    if not matched:
        matched = child_chunks_for_locator(chunks, ref.locator)
    if not matched:
        return _quote_fallback(ref, kind=kind, note=note, canonical=canonical)
    entry = _base_entry(ref, kind, note, canonical)
    entry["locator_resolved"] = True
    ...
    if ref.quote_hash:
        current_hash = locator_hash_for_ref(chunks, ref.locator)
...
def _resolve_video_ref(ref, note, canonical: dict[str, Any], chunks: list[SourceChunk]) -> dict[str, Any]:
    ...
    video_id = source_youtube_video_id(uri)
    ...
    matched = caption_chunks_for_time_range(chunks, f"t={start:.1f}-{end:.1f}" if end is not None else ref.locator)
