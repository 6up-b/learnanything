...
def request_probe_instance_surfaces(
    client: StructuredTransport, context: ProbeInstanceContext
) -> ProbeInstanceSurfaces:
    """Generate probe surfaces through the shared structured transport."""

    return execute_structured_operation(
        client,
        purpose="probe_instance_surfaces",
        prompt=probe_instance_surfaces_prompt(context),
        result_model=ProbeInstanceSurfaces,
    )


def request_probe_family_trials(
    client: StructuredTransport, context: ProbeFamilyTrialsContext
) -> ProbeFamilyTrials:
    """Generate planted family trials through the shared transport."""

    return execute_structured_operation(
        client,
        purpose="probe_family_trials",
        prompt=probe_family_trials_prompt(context),
        result_model=ProbeFamilyTrials,
    )
...
def _misconception_error_types(vault: LoadedVault) -> list[str]:
    ...
    if is_canonical_state_vault(vault):
        from learnloop.diagnosis.error_taxonomy_map import MECHANISM_IS_MISCONCEPTION
...
def _llm_prompt_version() -> str:
    from learnloop.diagnosis.ai_contracts import PROBE_INSTANCE_PROMPT_VERSION
...
def llm_instance_payloads(
    ...
    *,
    ...
    try:
        result = request_probe_instance_surfaces(ai_client, context)  # type: ignore[arg-type]
...
def generate_instances_for_episode(
    ...
    *,
    ...
    from learnloop.diagnosis.probe_episodes import eligible_instruments
...
def run_llm_family_gate(
    ...
    *,
    ...
    provider lacks structured completion.
    """

    from learnloop.ai.errors import CodexUnavailable
    from learnloop.diagnosis.probe_families import PlantedTrial, run_family_admission_gate
    ...
    try:
        result = request_probe_family_trials(ai_client, context)  # type: ignore[arg-type]
    except CodexUnavailable:
        return None
