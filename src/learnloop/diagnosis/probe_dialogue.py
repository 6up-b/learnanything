...
def request_probe_dialogue_turn(
    client: StructuredTransport, context: ProbeDialogueTurnContext
) -> ProbeDialogueTurn:
    """Generate one adaptive dialogue turn through the shared transport."""

    return execute_structured_operation(
        client,
        purpose="probe_dialogue_turn",
        prompt=probe_dialogue_turn_prompt(context),
        result_model=ProbeDialogueTurn,
    )
...
def next_dialogue_turn(
    ...
    *,
    ...
    When ``ai_client`` supports structured completion, the turn surface is
    generated adaptively — conditioned on the learner's prior committed
...
def _dialogue_prompt_version() -> str:
    from learnloop.diagnosis.ai_contracts import PROBE_DIALOGUE_TURN_PROMPT_VERSION
...
def _adaptive_turn_surface(
    ...
    *,
    ...
    try:
        turn = request_probe_dialogue_turn(ai_client, context)  # type: ignore[arg-type]
