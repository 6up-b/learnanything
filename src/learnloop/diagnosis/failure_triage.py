...
from learnloop.attempts import calibration_streams as CS
    ...
    *,
    ...
    if decisive_reason is not None:
        ...
        event = repository.append_failure_triage_event(
            ...
            inputs_snapshot_json=canonical_json(decision_payload),
            routing_prior_json=canonical_json(dict(routing_prior)) if routing_prior else None,
            auto_committed=True,
    ...
    event = repository.append_failure_triage_event(
        ...
        alternatives_json=canonical_json(alternatives),
        inputs_snapshot_json=canonical_json(decision_payload),
        routing_prior_json=canonical_json(dict(routing_prior)) if routing_prior else None,
