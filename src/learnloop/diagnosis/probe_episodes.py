...
Legacy ``probe_<lo_id>`` phases (``learnloop.diagnosis.probes``,
``lo_probe_state``) are
...
from learnloop.diagnosis.probe_hypotheses import (
    H_OTHER,
...
from learnloop.diagnosis.probe_robust import use_robust_probe
...
def enter_episode(
    ...
    *,
    ...
    if goal_id is not None:
        from learnloop.goals.goal_contracts import resolve_head
    ...
    from learnloop.diagnosis.probe_robust import resolve_episode_channel, use_robust_probe
...
def _record_generation_need(
    ...
    *,
    ...
    from learnloop.diagnosis.probe_instance_generation import applicable_families
...
def eligible_instruments(
    ...
    *,
    ...
    from learnloop.scheduling import controller_ownership as _ownership
...
def _robust_channel_pin(
    ...
    if episode.calibration_model_hash is None:
        return None
    from learnloop.diagnosis.probe_robust import (
        instrument_ensemble,
        load_pinned_channel,
    )
    from learnloop.diagnosis.robust_composition import robust_eig_per_second
...
def record_episode_evidence(
    ...
    *,
    ...
    if refreshed is None or refreshed.status != "in_progress":
        return None
    from learnloop.diagnosis.probe_blocks import block_complete, end_diagnostic_block
...
def _robust_observation_snapshot(
    ...
    *,
    ...
    if episode.calibration_model_hash is None:
        return None
    from learnloop.diagnosis.probe_robust import load_pinned_channel, pinned_decision_posterior
    ...
    *,
    ...
    try:
        ...
        if instrument is not None and use_robust_probe(vault):
            ...
            from learnloop.attempts.outcome_schemas import BUILTIN_SCHEMAS
...
def _plausible_actions_agree(
    ...
    # A singleton is posterior concentration, not action equivalence.  Let the
    # ordinary posterior-stop threshold decide whether that concentration is
    # strong enough; otherwise this path becomes a second, weaker confidence
    # stop whenever every runner-up falls just below the plausibility cutoff.
    if len(plausible) < 2:
        return False
