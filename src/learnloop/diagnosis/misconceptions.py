...
def request_misconception_match(
    client: OperationClient,
    context: MisconceptionMatchContext,
) -> MisconceptionMatch:
    """Run the feature-owned misconception matching operation."""

    return execute_structured_operation(
        client,
        purpose="misconception_match",
        prompt=misconception_match_prompt(context),
        result_model=MisconceptionMatch,
        legacy_capability="misconception_match",
...
def _match_misconception(
    ...
    *,
    ...
    Prefers the configured structured provider when available; otherwise falls
    back to a deterministic normalized-text match. Never dedupes by error type
    (spec §2.2.2).
    """
    ...
    if ai_client is not None:
        ...
        try:
            result = request_misconception_match(ai_client, context)  # type: ignore[arg-type]
        except Exception:
            result = None
    ...
    for row in candidates:
        if normalize_text(row.statement) == target:
            return row.id
    return None
    ...
    *,
    ...
    for event in events:
        ...
        if misconception_id is None:
            ...
            from learnloop.diagnosis.probe_episodes import maybe_reprobe_for_misconception
...
def _probe_signature_reproduced(candidate: dict, attempt: dict | None) -> bool:
    if str((attempt or {}).get("attempt_type") or "") != "diagnostic_probe":
        return False
    signature = normalize_text(str(candidate.get("signature") or ""))
    answer = normalize_text(str((attempt or {}).get("learner_answer_md") or ""))
...
def _promotion_reason(
    ...
    arrives days later. They live in ``learnloop.tutor.durable_promotion``, which
    re-drives ``promote_candidate`` for the same holding-pen candidate. Adding
...
def promote_candidate_if_independent(
    ...
    *,
    ...
    augmentation §8) and writes through the same ``promote_candidate`` door, so
...
def normalize_and_resolve_attempt(
    ...
    *,
    ...
    from learnloop.tutor.durable_promotion import sweep_late_promotion_evidence
