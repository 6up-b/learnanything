...
* ``no_profile_applies`` is a sibling arm of ``matched`` in one closed
  vocabulary
  (``learnloop.attempts.ai_contracts.DiscriminationProfileMatch``), so rejection
  cannot
...
both this module and ``learnloop.content.authoring.persona_gate`` use, so the
belief a profile
...
def profile_match_fill_rate(
    ...
    * **The verdict is named, not inferred.** ``detail.verdict`` is one of
    ...
    AVAILABILITY (the module-wide discipline in
