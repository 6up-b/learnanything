...
def diagnosis_receipt(repository: Repository, attempt_id: str) -> dict[str, Any] | None:
...
def selected_repair(receipt: dict[str, Any] | None) -> dict[str, Any] | None:
...
def guided_redo_available(repository: Repository, attempt_id: str) -> bool:
    ...
    selected = selected_repair(diagnosis_receipt(repository, attempt_id))
    if selected is None:
        return False
    return bool(_preserved_prefix(selected).strip())


def item_step_checkpoint_ids(repair_class: dict[str, Any] | None) -> list[str]:
...
def _bind_episode_to_redo(
    ...
    *,
    ...
    from learnloop.diagnosis.remediation import episode_case, _rank_items
...
def start_guided_redo(
    ...
    *,
    ...
    selected = selected_repair(receipt)
    if selected is None:
        raise GuidedRedoUnavailable(
            "no_repair_selection",
    ...
    ] or item_step_checkpoint_ids(repair_class)
