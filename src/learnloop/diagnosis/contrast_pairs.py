...
``learnloop.content.authoring.contract_commissioning`` deliberately — same
shape, same discipline:
...
1. **both members independently in the target difficulty band** — via the
   existing ``practice_generation.success_band_difficulty`` inversion, so the
...
4. the §3.0 persona clause (the belief-holder must fail exactly one member) which
   ``learnloop.content.authoring.persona_gate`` already owns and this module
   does not duplicate.
...
from learnloop.diagnosis.discrimination_profiles import profiles_by_facet
...
class ContrastPairDisposition(StrEnum):
    ...
    ``DEFER_RUNG_COMMISSIONING``
        ...
        isolated capability, which
        ``learnloop.content.authoring.contract_commissioning`` already
...
def commission_contrast_pairs(
    ...
    *,
    ...
    from learnloop.learner.identifiability import load_misconception_records
    ...
    for sid in subject_ids:
        ...
        records = (
            load_misconception_records(vault, repository, scoped)
...
def judge_pair(
    ...
    *,
    ...
    by ``practice_generation.success_band_difficulty`` — the existing inversion
