...
def request_diagnostic_trials(
    client: StructuredTransport,
    context: Any,
) -> DiagnosticTrials:
    """Generate planted diagnostic trials through the structured transport."""

    return execute_structured_operation(
        client,
        purpose="diagnostic_trials",
        prompt=diagnostic_trials_prompt(context),
        result_model=DiagnosticTrials,
    )


def request_diagnostic_fire(
    client: StructuredTransport,
    **context: Any,
) -> bool:
    """Semantically judge one simulated diagnostic answer in memory."""

    judgment = execute_structured_operation(
        client,
        purpose="grade_diagnostic_fire",
        prompt=diagnostic_fire_prompt(context),
        result_model=DiagnosticFireJudgment,
    )
    return bool(judgment.fires)
...
#: Public alias. §3.0's shared authoring gate
#: (``learnloop.content.authoring.persona_gate``) and the D2 mint gate
#: (``learnloop.content.synthesis.facet_mint_gate``) decide "categorically distinct"
...
def run_discrimination_gate(
    ...
    *,
    ...
    def _fire(answer: str) -> bool:
        if grading_client is not None and grading_client.supports(STRUCTURED_COMPLETION):
            return request_diagnostic_fire(
                grading_client,
                item=item,
    ...
    # and a structured provider is wired through. The
    ...
    supports_trials = bool(
        grading_client is not None
        and grading_client.supports(STRUCTURED_COMPLETION)
    )
    notes: list[str] = []
    llm_trials_ran = False
    if llm_trials > 0 and supports_trials and det_accepted:
        try:
            llm_result = request_diagnostic_trials(
                grading_client,
                _diagnostic_trials_context(
                    item,
                    misconception,
                    expected=expected,
                    misconception_consistent=misconception_consistent,
