...
from learnloop.ai.schemas import CandidateCause
...
from learnloop.diagnosis.diagnosis_adjudication import anchor_key
from learnloop.attempts.grading import (
    build_grading_context,
    request_grading_proposal,
    resolved_rubric,
)
from learnloop.diagnosis.diagnostic_gate import request_diagnostic_trials
from learnloop.content.authoring.persona_realism import (
...
def run_diagnosis_samples(
    ...
    *,
    ...
    if sample_count == 1:
        only = _sample(request_grading_proposal(client, context), 0)
    ...
    samples = tuple(
        _sample(request_grading_proposal(client, context), index)
...
def _generated_trace(
    ...
    if (
        case.should_abstain
        or not generator_client.supports(STRUCTURED_COMPLETION)
    ):
        return case.learner_trace_md
    trials = request_diagnostic_trials(
        generator_client,
        {
            "n_trials": 1,
