...
def cold_success_trajectories(
    ...
    *,
    ...
    from learnloop.diagnosis.causal_activity_policy import attempt_counts_as_assisted
...
def harmful_write_rate(repository: Repository) -> Metric:
    ...
    from learnloop.learner.surfaced_beliefs import typed_withdrawal_reason
...
def cells_cleared_per_question(vault: LoadedVault, repository: Repository) -> Metric:
    ...
    from learnloop.learner.contract_reachability import contract_cells
...
def _certified_at_cutoff(
    ...
    # `prune_rows` walks the FK graph clearing non-cascading referrers. Imported
    ...
    from learnloop.goals.goal_series import prune_rows
    ...
    try:
        scratch = Repository.attach(scratch_path, read_only=False)
        with scratch.pinned():
...
# ---------------------------------------------------------------------------
#
# Unblocked by plan item 1.2 (migration 131 + `learnloop.ai.usage` +
# `agent_runs.finish_agent_run`), and producible only because BOTH grading paths
# in ``learnloop.attempts.attempts`` now carry tokens.
#
...
def planted_vs_adjudicated_agreement(repository: Repository) -> Metric:
    ...
    from learnloop.diagnosis.diagnosis_adjudication import adjudicated_ground_truth
