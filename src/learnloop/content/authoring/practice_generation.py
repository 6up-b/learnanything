...
from learnloop.substrate.activity_patterns import LEGACY_UNMAPPED, map_capability
...
from learnloop.learner.facet_state_reader import facet_recall_states_for_lo
from learnloop.learner.mastery import covering_learner_claim, display_mastery
...
from learnloop.content.synthesis.synthesis_gates import GateDiagnostic
...
class PracticeExpansionTarget:
    ...
    # Depth-rung target (``learnloop.curriculum.depth_rungs``): the waypoint in
    # capability ×
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        if self.rung is not None:
            from learnloop.curriculum.depth_rungs import rung_float_proxies
    ...
    for learning_object in sorted(vault.learning_objects.values(), key=lambda lo: lo.id):
        ...
        targets.append(
            PracticeExpansionTarget(
                ...
                mastery_mean=mastery_mean,
                recommended_difficulty_band=success_band_difficulty(
                    ability_logit(ability),
            ...
            DiagnosticPracticeTarget(
                ...
                recommended_difficulty_band=_guard_degenerate_band(
                    success_band_difficulty(
                        ability_logit(_ability_estimate(facet_means, mastery_mean)),
...
def generate_post_probe_practice_proposal(
    ...
    *,
    ...
    # SAME target band the rung gate already uses (`success_band_difficulty`'s
...
class _RungGate:
    ...
    def __init__(self, repository: Repository, plan: PracticeExpansionPlan):
        from learnloop.substrate.activity_patterns import ensure_capability_alias_registry
...
#: them from the posterior
#: (``learnloop.content.authoring.conjunctive_items.conjunctive_fit``).
_CONJUNCTIVE_ITEM_RULE = (
...
def ability_logit(ability: float | None) -> float:
...
def _difficulty_for_success(
    ...
    *,
    ...
    Inverts the mastery channel's 2PL link (``learnloop.learner.mastery``):
