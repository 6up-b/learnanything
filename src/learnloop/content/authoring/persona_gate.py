...
(``learnloop.diagnosis.diagnostic_gate.run_discrimination_gate``), and one
caller enforces
...
(:func:`~learnloop.diagnosis.diagnostic_gate.normalize_answer`). Deliberately
...
from learnloop.diagnosis.diagnostic_gate import normalize_answer, request_diagnostic_fire
from learnloop.diagnosis.discrimination_profiles import payload_profiles
...
def _fires(
    ...
    *,
    ...
    supporting structured completion uses ``diagnostic_gate``'s public
    operation to replace the string comparison with a semantic judgement.
    """
...
class PersonaGate:
    ...
    def _judge_contrast_pair(
        ...
        semantic_oracle = bool(
            self._grading_client is not None
            and self._grading_client.supports(STRUCTURED_COMPLETION)
        )
        trials = [
            PersonaTrial(
...
def gate_precision(
    ...
    * Stage 7 **B1**'s planted-vs-adjudicated side, which
    ...
    AVAILABILITY (same discipline as ``learnloop.diagnosis.scoreboard``)
    --------------------------------------------------------
