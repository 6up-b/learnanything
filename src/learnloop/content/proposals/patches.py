...
def apply_accepted_items(
    ...
    *,
    ...
    (``learnloop.content.proposals.apply_protocol``, §10.2): a durable intent
    commits to SQLite first,
...
def _apply_accepted_locked(
    ...
    from learnloop.content.proposals.apply_protocol import (
    ...
    if content_lo_ids:
        from learnloop.substrate.replay import record_content_recalibration
...
def _reject_unregistered_facets(vault: LoadedVault, entity_id: str, data: dict[str, Any]) -> None:
    ...
    if not vault.evidence_facets:
        return
    from learnloop.learner.capability_mapping import unregistered_facet_errors
