...
from learnloop.ai.errors import CodexInterrupted, CodexTurnTimeout, CodexUnavailable
...
from learnloop.diagnosis.diagnostic_gate import GateResult, run_discrimination_gate
from learnloop.ids import new_ulid
from learnloop.ai.usage import consume_client_usage
from learnloop.content.proposals.patches import PatchApplyResult, apply_accepted_items, reject_applied_items
...
def request_authoring_proposal(
    ...
    """Run the feature-owned authoring operation on a structured provider."""

    return execute_structured_operation(
        client,
        purpose="authoring",
        prompt=authoring_prompt(context),
        result_model=AuthoringProposal,
        legacy_capability="authoring",
...
def authoring_context_stats(context: AuthoringContext) -> dict[str, Any]:
    ...
    prompt = authoring_prompt(context)
    schema_json = json.dumps(strict_output_schema(AuthoringProposal), sort_keys=True, ensure_ascii=False)
...
def generate_authoring_proposal(
    ...
    *,
    ...
    """Run authoring generation through a AIProviderClient and persist the result.
    ...
    and BEFORE persist + ``auto_apply_rows``, so a caller can downgrade a row's
    ...
    try:
        proposal = request_authoring_proposal(codex_client, context)
    ...
    rows = [
        proposal_item_row(item, now, vault=vault, proposal=proposal, provider=provider)
    ...
    auto_apply_rows(root, patch_id, rows)
    return patch_id
...
def _diagnostic_proposal_item_row(
    ...
    *,
    ...
    row = proposal_item_row(item, now, vault=vault, proposal=proposal, provider=provider)
...
def generate_diagnostic_proposal(
    ...
    *,
    ...
    try:
        proposal = request_authoring_proposal(client, context)
    except (CodexUnavailable, TimeoutError, ValueError) as exc:
        finish_agent_run(
...
def _repair_invalid_proposal_items(
    ...
    *,
    ...
    try:
        repaired = request_authoring_proposal(
            codex_client,
            replace(context, instructions=instructions),
        )
    ...
    merged_rows = [
        proposal_item_row(item, now, vault=vault, proposal=merged, provider=provider)
        for item in merged.items
    ]
    return merged, merged_rows


def auto_apply_rows(root: Path, patch_id: str, rows: list[dict[str, Any]]) -> None:
...
def _instrument_gate_errors(
    ...
    from learnloop.content.authoring.authoring_gates import selected_response_reasons
    from learnloop.content.authoring.persona_gate import GateDecision, PersonaGate
...
# Kept as a local literal
# (matches ``learnloop.tutor.teach_back.TEACH_BACK_PRACTICE_MODE``)
