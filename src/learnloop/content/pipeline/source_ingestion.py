...
from learnloop.content.proposals.proposals import auto_apply_rows, proposal_item_row
...
def request_canonical_ingest(
    client: AIProviderClient,
    context: CanonicalIngestContext,
) -> AuthoringProposal:
    """Run the feature-owned canonical-ingest operation."""

    return execute_structured_operation(
        client,
        purpose="canonical_ingest",
        prompt=canonical_ingest_prompt(context),
        result_model=AuthoringProposal,
        legacy_capability="canonical_ingest",
...
def ingest_canonical_source(
    ...
    *,
    ...
    retry_client: AIProviderClient | None = None,
    ...
    try:
        ...
        rows = [
            proposal_item_row(item, now, vault=vault_for_validation, proposal=merged, provider=provider_fields["provider"] or "codex")
        ...
        auto_apply_rows(vault.root, patch_id, rows)
...
def _apply_instrument_gates(rows: list[dict[str, Any]], vault, repository, grading_client) -> None:
    ...
    `warning`, and both lose `_auto_apply`, so `auto_apply_rows` routes them
...
def analyze_source_change(
    ...
    *,
    ...
    previous_chunks = {
        note.id: chunks_for_note_body(source.kind, note.body)
    ...
    for entity_type, entity_id, subject, source_refs in _grounded_entity_refs(vault):
        for ref in source_refs:
            ...
            new_ref_hash = locator_hash_for_ref(new_chunks, ref.locator)
            old_ref_hash = locator_hash_for_ref(old_chunks, ref.locator)
...
def _fetch_youtube_transcript(source: str, *, allow_auto_captions: bool, clock: Clock | None) -> FetchResult:
    video_id = source_youtube_video_id(source)
    if video_id is None:
        raise SourceIngestionError("unsupported or inaccessible source")
    try:
...
def chunks_for_note_body(kind: SourceKind, body: str) -> list[SourceChunk]:
    if kind == "youtube_video":
        chunks: list[SourceChunk] = []
...
def _locator_resolves(chunks: list[SourceChunk], locator: str) -> bool:
    if locator in {chunk.locator for chunk in chunks}:
        return True
    if child_chunks_for_locator(chunks, locator):
        return True
    return bool(caption_chunks_for_time_range(chunks, locator))


def locator_hash_for_ref(chunks: list[SourceChunk], locator: str | None) -> str | None:
    if locator is None:
        return None
    for chunk in chunks:
        if chunk.locator == locator:
            return hashlib.sha256(chunk.text.strip().encode("utf-8")).hexdigest()
    child_chunks = child_chunks_for_locator(chunks, locator)
    if child_chunks:
        text = "\n".join(chunk.text.strip() for chunk in child_chunks)
        return hashlib.sha256(text.encode("utf-8")).hexdigest()
    caption_chunks = caption_chunks_for_time_range(chunks, locator)
...
def child_chunks_for_locator(chunks: list[SourceChunk], locator: str) -> list[SourceChunk]:
...
def caption_chunks_for_time_range(chunks: list[SourceChunk], locator: str) -> list[SourceChunk]:
...
def source_youtube_video_id(source: str) -> str | None:
    parsed = urllib.parse.urlparse(source)
