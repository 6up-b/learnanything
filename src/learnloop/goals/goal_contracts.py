...
def canonicalize_body(body: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_capabilities": sorted(list(body.get("required_capabilities") or []), key=canonical_json),
        "task_types": sorted(list(body.get("task_types") or []), key=canonical_json),
...
def support_hash(body: Mapping[str, Any]) -> str:
    return canonical_hash(_support_subset(body))
...
def compute_change_class(prev: Mapping[str, Any], new: Mapping[str, Any]) -> str:
    ...
    if canonical_json(_support_subset(prev)) != canonical_json(_support_subset(new)):
        return "support_change"
    if canonical_json(_evaluation_subset(prev)) != canonical_json(_evaluation_subset(new)):
        return "evaluation_change"
    if canonical_json(_exemplar_weights(prev)) != canonical_json(_exemplar_weights(new)):
        return "reweight"
    return "metadata"
...
def _support_diff_touches_envelope(
    ...
    for leaf_key in set(prev_support) | set(new_support):
        ...
        if canonical_json(pv) == canonical_json(nv):
            continue
    ...
    *,
    ...
    if len(exemplars) < 1:
        repository.insert_goal_contract_draft(
            ...
            proposed_contract_json=canonical_json(body),
    ...
    result = repository.append_goal_contract_version(
        ...
        version=1,
        predecessor_version_id=None,
        contract_json=canonical_json(body),
...
def append_successor(
    ...
    *,
    ...
    result = repository.append_goal_contract_version(
        ...
        contract_json=canonical_json(new_body),
    ...
    *,
    ...
    def _reject(rejection_reason: str, requires: str) -> Draft:
        draft_id = repository.insert_goal_contract_draft(
            goal_id=goal_id,
            predecessor_version_id=prev["id"],
            proposed_contract_json=canonical_json(new_body),
            proposed_change_class="authorized_depth_step",
            rejection_reason=rejection_reason,
            requires=requires,
            author=author,
            evidence_receipt_json=canonical_json(progression_decision) if progression_decision else None,
            clock=clock,
        )
        return Draft(
            id=draft_id,
    ...
    for leaf_key in set(prev_support) | set(new_support):
        ...
        if canonical_json(prev_val) == canonical_json(new_val):
            continue
    ...
    result = repository.append_goal_contract_version(
        ...
        evidence_receipt_json=canonical_json(progression_decision),
        burden_delta_json=canonical_json(edge.get("burden_delta") or bounds.get("cumulative_burden") or {}),
        author=author,
...
def detect_contract_drift(
    ...
    for field_name in _YAML_DRIFT_FIELDS:
        if canonical_json(yaml_body.get(field_name)) != canonical_json(head_body.get(field_name)):
