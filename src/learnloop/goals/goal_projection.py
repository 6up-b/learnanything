...
from learnloop.learner.blueprint_projection import LoReadiness, project_lo_readiness
from learnloop.learner.contract_reachability import (
    ReachabilityVerdict,
    analyze_contract_reachability,
)
from learnloop.learner.facet_diagnostics import facet_state_label, scope_facets
...
from learnloop.goals.goal_certification import facet_demonstration
from learnloop.learner.mastery import covering_learner_claim
from learnloop.learner.measurement_state import UNKNOWN as MEASUREMENT_UNKNOWN
from learnloop.learner.measurement_state import classify_measurement_state
...
def project_lo_blueprint_readiness(
    vault: LoadedVault,
    learning_object_id: str,
...
def _blueprint_readiness_by_lo(
    ...
    *,
    ...
    for learning_object_id in sorted(resolve_goal_scope(vault, goal, repository)):
        lo_readiness = project_lo_blueprint_readiness(
            vault,
            learning_object_id,
...
def facet_projections_at(
    ...
    *,
    ...
    """Per-facet do-nothing projection to ``at`` (the horizon overridden).

    Shared read helper: ``projected_ready_mean_at`` and the F7 welcome-back /
    decay-pressure surfaces all need per-facet ``predicted_at_horizon`` at an
    arbitrary future instant, not just the goal's own horizon.
    """

    now = (clock or SystemClock()).now().astimezone(UTC)
    return _facet_projections(
        vault,
        repository,
        goal,
        now=now,
        ...
        mastery_states=repository.mastery_states(),
        fsrs_weights=resolve_fsrs_weights(repository),
        include_demonstration=include_demonstration,
    )
...
def projected_ready_mean_at(
    ...
    *,
    ...
    facets = facet_projections_at(vault, repository, goal, at, clock=clock)
