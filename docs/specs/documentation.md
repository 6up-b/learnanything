...
Optional extra: `--extra pdf` installs Marker for high-fidelity PDF extraction (heavy — it pulls in torch). It is runtime-detected; without it PDF extraction falls back to pypdf. Manim (explainer animations) is a core dependency and needs no extra.
...
Each concept's inspector has an "Explainer animation" section. Generating one sends the concept description to the `[ai.routing] animation` model, which writes a Manim Community Edition scene; the code is validated against an import/builtin allowlist and rendered locally by manim (installed with LearnLoop's Python environment), then plays inline. Every generation requires an explicit consent click because AI-written code executes on your machine. On Linux the render runs inside a mandatory [bubblewrap](https://github.com/containers/bubblewrap) sandbox — no network, read-only system and interpreter mounts, a throwaway scratch directory as the only writable path — and rendering refuses with an install hint if `bwrap` is missing. On macOS/Windows (no bubblewrap) the render falls back to the constrained subprocess (temp directory, timeout, minimal environment); there the AST validation is hardening, not a guarantee. Failed generations keep the scene code and renderer output for debugging, and one automatic repair round-trip is attempted. Configuration lives under `[animation]` (quality, timeout, LaTeX availability, kill-switch).
...
You can then choose introductory, standard, or deep coverage; include or exclude topics; state a level or notation preference; and add free-form scope/authoring instructions such as “treat this as a narrow adjunct; add one learning object without restructuring the existing map.” For exam prep, the wizard also collects goal parameters. The brief asks for a starting level (`new to this`, `some exposure`, `comfortable`, `strong background`), which seeds a global learner claim (section 11.1), and exposes a separate practice-timing choice: `upfront`, or `as you read` — the items-off bootstrap, where the map ships with zero practice items and completing a guide section triggers generation for the learning objects that section supports. Reader participation is configured separately from this timing choice.

For a small source just outside the main curriculum, choose **Enrichment · interesting adjunct** in Quick Add. This is an authoring preset rather than a new authority role: it keeps the membership role at `reference`, keeps Reader participation on, requests one or two upfront practice items, prefers attaching the source to existing curriculum, permits at most one focused new learning object when no home exists, and forbids restructuring the existing map. The generated scope remains editable through **customize brief**.
