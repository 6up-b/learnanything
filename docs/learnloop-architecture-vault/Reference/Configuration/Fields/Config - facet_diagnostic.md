...
aliases:
  - "learnloop.toml facet_diagnostic"
  - "[facet_diagnostic] configuration"
...
tags:
  ...
  - "learnloop/configuration/section/facet-diagnostic"
...
> This catalog was generated by parsing the current decision-only template and validating it through `LearnLoopConfig`. It contains 4 leaf values under `facet_diagnostic`.
...
|---|---|---|---|---|---|---|
| `facet_diagnostic.tau_facet_failed` | `0.4` | number | modeled default or validator seed | Sets the decision cutoff for facet failed in facet failure and uncertainty classification. | **ACTIVE** — Declared by the canonical typed configuration; exact runtime consumers are cited when a statically resolvable path exists. | `src/learnloop/config/schema.py:510` |
| `facet_diagnostic.tau_facet_uncertain_variance` | `0.15` | number | modeled default or validator seed | Sets the decision cutoff for facet uncertain variance in facet failure and uncertainty classification. | **ACTIVE** — Declared by the canonical typed configuration; exact runtime consumers are cited when a statically resolvable path exists. | `src/learnloop/config/schema.py:511` |
| `facet_diagnostic.hedge_uncertainty_floor` | `0.5` | number | modeled default or validator seed | Controls hedge uncertainty floor in facet failure and uncertainty classification, using the typed value shown here as the fresh-vault default. | **ACTIVE** — Declared by the canonical typed configuration; exact runtime consumers are cited when a statically resolvable path exists. | `src/learnloop/config/schema.py:512`; `src/learnloop/learner/facet_diagnostics.py:535` |
| `facet_diagnostic.facet_resolved_threshold` | `0.1` | number | modeled default or validator seed | Sets the decision cutoff for facet resolved in facet failure and uncertainty classification. | **ACTIVE** — Declared by the canonical typed configuration; exact runtime consumers are cited when a statically resolvable path exists. | `src/learnloop/config/schema.py:513`; `src/learnloop/learner/facet_diagnostics.py:538` |
