...
aliases:
  - "state.sqlite probe_state_segments"
  - "table probe_state_segments"
schema_head: 157
table_name: "probe_state_segments"
table_role: "workflow"
...
| `sequence` | `INTEGER` | yes | — | — | Stored value |
| `reason` | `TEXT` | yes | — | — | Stored value |
| `opened_by_attempt_id` | `TEXT` | no | — | — | Application-validated soft reference |
...
- `idx_probe_state_segments_order` on `learning_object_id`, `sequence` (unique).
- `idx_probe_state_segments_lo` on `learning_object_id`, `sequence`.
- `sqlite_autoindex_probe_state_segments_1` on `id` (unique).
...
- `Repository.state_segments_for_learning_object()`
