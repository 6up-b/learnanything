...
aliases:
  - "state.sqlite diagnostic_pack_events"
  - "table diagnostic_pack_events"
schema_head: 157
table_name: "diagnostic_pack_events"
table_role: "receipt"
...
> Append-only admission/review ledger (U-028 artifacts-not-API-calls): every register/review/admit/reject/activate decision is a durable, reviewable record.
...
| `card_slug` | `TEXT` | no | — | — | Stored value |
...
- `idx_diag_pack_events_pack` on `pack_id`, `created_at`.
- `sqlite_autoindex_diagnostic_pack_events_1` on `id` (unique).

## Who calls it

### Repository access surface

- `Repository.diagnostic_pack_events_for()`
- `Repository.ensure_diagnostic_pack()`
